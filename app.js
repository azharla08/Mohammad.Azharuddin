...
// Ensure "Download CV" works as a file download from the same folder (even if markup changes)
document.addEventListener('DOMContentLoaded', function() {
    // If a download button exists as <button>, attach a download action
    const candidates = Array.from(document.querySelectorAll('.btn, button, a'))
        .filter(el => (el.textContent || '').trim().toLowerCase() === 'download cv');

    candidates.forEach(el => {
        // If it's already a link to the PDF, nothing to do
        if (el.tagName === 'A') {
            const href = el.getAttribute('href') || '';
            if (!href || href === '#') {
                el.setAttribute('href', 'Mohammad_Azharuddin_CV.pdf');
            }
            el.setAttribute('download', '');
            return;
        }

        // If it's a button, prevent print behavior and trigger a file download
        el.addEventListener('click', function(e) {
            e.preventDefault();
            const a = document.createElement('a');
            a.href = 'Mohammad_Azharuddin_CV.pdf';
